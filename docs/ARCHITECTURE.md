...
| Reset password            | `/auth/callback` → `/reset-password` | The emailed link signs the user in with a **recovery session**. The page and action only accept a session whose JWT `amr` has `recovery` from the last hour (`lib/recovery.ts`); otherwise it's a normal session and must use "change password". |
...
- Authentication hardening (Sprint 1):
  - Password reset only in a recent recovery session (JWT `amr`), so a stolen ordinary session
    can't set a new password without the current one.
...
| 16  | Reset password only in a recent recovery session (JWT `amr`)                   | Otherwise `/reset-password` would be a "change password without the current one" backdoor for any stolen session.                                                    |
