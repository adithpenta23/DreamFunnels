...
- **PostgREST clock workaround** (`lib/supabase/postgrest-retry.ts`): PostgREST before v16.3 /
  v14.18 can reject a token minted in the current second as "JWT issued at future" (PGRST303),
  which hit new users right after sign-up. The server client retries such a request once after a
  second; the JWT is checked before any query runs, so this is safe for writes. Remove it once
  every environment runs a fixed PostgREST (the Supabase CLI still ships v16.2).
...
| 21  | Retry PGRST303 once in the app instead of pinning PostgREST images             | Fixes new users' first request in every environment (local, CI, hosted), not just CI; tiny and removable once PostgREST ≥ 16.3 ships everywhere.                                                           |
