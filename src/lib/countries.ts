/**
 * ISO 3166-1 alpha-2 country codes (all 249 assigned codes). Addresses store
 * the code ("US"); names come from Intl at render time. Build option lists on
 * the server and pass them down, so every browser shows the same labels.
 */
...
export type CountryOption = { value: string; label: string; keywords: string }

/** Every country, alphabetical by name in `locale`. */
